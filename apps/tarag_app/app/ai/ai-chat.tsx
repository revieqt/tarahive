...
import { ChatArea, ChatHeader, ChatField, ChatBubble } from '@/components/Chat';
...
import { TARA_AI_SUGGESTIONS } from '@/constants/Config';
...
export default function AiChatScreen() {
  ...
  const headerOptions: React.ReactNode[] = [
    ...
    >
      <ThemedIcons name="trash-can" size={18} />
      <ThemedText style={{ marginLeft: 10 }}>Clear Chat</ThemedText>
    </TouchableOpacity>,
    <TouchableOpacity
      key="toggle-tts"
      style={{ flexDirection: 'row', alignItems: 'center' }}
        ...
        {!hasUserMessages ? (
          ...
          >
            ...
            <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center'}}>
              ...
              <ThemedText style={{ textAlign: 'center', opacity: 0.7, marginTop: 10, paddingHorizontal: 16}}>
            ...
            >
              {rateLimitError && (
                <ThemedText style={{ color: '#ef4444', padding: 8, textAlign: 'center', marginBottom: 8 }}>
                  {rateLimitError}
                </ThemedText>
              )}
            </ChatField>
              ...
              >
                    ...
                    return (
                      <View key={msg.id} style={{ paddingVertical: 30, alignItems: 'flex-start', maxWidth: '95%', marginBottom: isLastMessage ? 60 : 0, gap: 8 }}>
                        ...
                        {msg.itineraryData && msg.type === 'itinerary' && (
                          ...
                          >
                            <View style={{flex: 1}}>
                              <ThemedText type='subtitle'>{msg.itineraryData?.title || 'View Itinerary'}</ThemedText>
                            ...
                            <ThemedIcons name="chevron-right" size={20} color='#ccc7' style={{ marginLeft: 10 }} />
                          </TouchableOpacity>
                        )}
                      </View>
                    );
                ...
                {isSending && (
                  <View style={{ paddingVertical: 30,padding: 10, opacity: 0.7 }}>
                    <ThemedText>Thinking{'.'.repeat(dotCount)}</ThemedText>
                  </View>
                )}
              </ScrollView>
            </ChatArea>
