    ...
    const initializeTheme = async () => {
      try {
        ...
        if (isInitialized && themeCache) {
          setTheme(themeCache);
          applyThemeToDocument(themeCache);
          setIsLoading(false);
          return;
        }
...
// Helper function to apply theme to document (for CSS variables, etc.)
const applyThemeToDocument = (theme: ThemeType): void => {
  const deviceTheme = getDeviceTheme();
  const actualTheme = theme === THEME_TYPES.DEVICE ? deviceTheme : theme;
  
  // Apply theme class to document root
  document.documentElement.setAttribute('data-theme', actualTheme);
  
  // Optional: Add dark/light class for Tailwind CSS
  if (actualTheme === 'dark') {
