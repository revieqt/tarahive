...
import { SendVerificationEmailJobData, Verify2FAEmailJobData, SendPasswordResetEmailJobData } from './auth.queue';
...
export const authWorker = new Worker(
  ...
  async (job) => {
    ...
    try {
      ...
      if (job.name === 'verify-2fa-email') {
        const data = job.data as Verify2FAEmailJobData;
        
        await sendEmail({
          to: data.email,
          subject: 'TaraG - Two-Factor Authentication Code',
          content: generate2FAEmailContent(data.code),
        });

        console.log(`✅ 2FA email sent to ${data.email}`);
        return { success: true, email: data.email };
      }
